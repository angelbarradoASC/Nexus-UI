...
function renderProspectingResults(items) {
    ...
    if (!items.length) {
        ...
        tbody.innerHTML = items.map((item) => {
            ...
            const crmState = item.crm_duplicate
                ? '<span class="pill pill-dup">duplicado</span>'
                : `<span class="crm-state-label crm-${escapeHtml(item.crm_state || "pendiente")}">${escapeHtml(item.crm_state || "pendiente")}</span>`;
...
async function pushValidResults() {
    ...
    _setCrmFeedback("Enviando al CRM...");
    try {
        ...
        _setCrmFeedback(n > 0 ? `${n} leads enviados al CRM.` : "0 leads nuevos (ya existian o no habia validos).");
    } catch (error) {
        _setCrmFeedback(`Error: ${error.message}`, true);
    } finally {
        if (btn) { btn.disabled = false; btn.textContent = "Enviar validos al CRM"; }
    }
}

async function pushSingleResult(resultId) {
    const message = document.getElementById("prospectingResultMessage");
    if (message) message.textContent = "Enviando al CRM...";
    try {
        ...
        if (message) {
            message.textContent = response.company_payload
                ? `${response.company_payload.name} enviado al CRM.`
                : "Lead enviado al CRM.";
        }
    } catch (error) {
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    document.getElementById("prospectingResultsBody")?.addEventListener("click", async (event) => {
        ...
        if (action === "push") await pushSingleResult(resultId);
